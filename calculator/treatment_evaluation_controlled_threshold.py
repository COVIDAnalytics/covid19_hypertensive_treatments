...
for data_version in ['train','test','validation','validation_cremona','validation_hope','validation_hope_italy']:
    ...
    for weighted_status in schemes:
        ...
        if weighted_status == 'weighted':
            ...
            # Redistribute treatments based on a threshold
            summary['Dif'] = abs(summary[treatment_list[0]]-summary[treatment_list[1]])
            
            #For those rows that the difference is less than the threshold, we default not to prescribe the drug            
            summary.loc[summary.Dif < threshold, 'Prescribe'] = treatment_list[1]
            summary.loc[summary.Dif < threshold, 'AverageProbability'] = summary.loc[summary.Dif < threshold, treatment_list[1]]

            
            summary = pd.concat([summary, Z, y],axis=1)
            summary['Match'] = [x.replace(' ','_') == y for x,y in zip(summary['REGIMEN'], summary['Prescribe'])]
        
            summary.to_csv(save_path+data_version+'_'+match_status+'_bypatient_summary_'+weighted_status+str(threshold)+'.csv')
            n_summary = summary
        else:
        ...
        prescription_summary.to_csv(save_path+data_version+'_'+match_status+'_bytreatment_summary_'+weighted_status+str(threshold)+'.csv')
        ...
        pr_table.to_csv(save_path+data_version+'_'+match_status+'_prescription_robustness_summary_'+weighted_status+str(threshold)+'.csv')
...
metrics_agg.to_csv(save_path+match_status+'_metrics_summary'+str(threshold)+'.csv')
