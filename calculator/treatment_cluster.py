...
train_inds = X['COUNTRY'] == 'SPAIN'
X_train, y_train = X.loc[train_inds,].drop('COUNTRY', axis=1), y[train_inds]
X_test, y_test = X.loc[-train_inds,].drop('COUNTRY', axis=1), y[-train_inds]

## HMW: Need to be all numeric (at least in current implementation. We could one-hot encode in R)
X_train = X_train.loc[:,ds.lab_cols+ds.vital_cols]
X_train = impute_missing(X_train)

# Train model
## HMW: get error that y_true is always 0 so can't calculate ROC
best_model, best_params = o.optimizer(algorithm, name_param, X_train, y_train, n_calls = 500, name_algo = name_algo)
