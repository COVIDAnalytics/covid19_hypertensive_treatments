...
loveplot_custom <- function (treatment_name, X_mat, t_id, c_id, v_line, legend_position = "topright") 
{
  X_mat_t = X_mat[t_id, ]
  ...
  std_dif_before = (X_mat_t_mean - X_mat_c_before_mean)/sqrt((X_mat_t_var + 
                                                                X_mat_c_before_var)/2)
  X_mat_c_after = X_mat[c_id, ]
  X_mat_c_after_mean = apply(X_mat_c_after, 2, mean)
  std_dif_after = (X_mat_t_mean - X_mat_c_after_mean)/sqrt((X_mat_t_var + 
                                                              X_mat_c_before_var)/2)
  ...
  points(abs_std_dif_after[n_aux:1], y = 1:ncol(X_mat), cex = 0.8, 
         pch = 8, col = "blue")
  legend(0.8,16, c("Before matching", "After matching"), 
         cex = 0.7, bty = "n", pch = c(0, 8), col = c("black", 
                                                      "blue"), 
